...
- [X] T063 [US5] Add the `prek-advisory` row to `tests/published_surface.toml`: check name `prek-advisory`, inputs `hook-stage` and `timeout-minutes` — no `cache-prek`, no `mise-version` — `contents: read` and `pull-requests: write`
- [X] T064 [US5] Create `.github/workflows/prek-advisory.yml` as a separate capability precisely because of its write demand, so a consumer can take the CI capability without granting write access (FR-003)
- [X] T079 [US5] **Executes before T065.** Add the tool pre-flight to `.github/workflows/prek-advisory.yml`, mirroring T017: `contracts/published-surface.md` lists this capability as needing both the package manager and the hook runner from the consumer's configuration, so both are checked before use and a missing one fails naming the tool, this capability, and where to declare it (FR-009). Omitting it here would leave the only capability that invokes consumer tools without a pre-flight
- [X] T065 [US5] Implement the whole-tree lint in `.github/workflows/prek-advisory.yml` with the finding non-blocking but the capability's own setup — including the lockfile check — still failing the check, and say so in the README: a green check means the lint ran, not that it passed (FR-047)
- [X] T066 [US5] Implement the idempotent comment in `.github/workflows/prek-advisory.yml`: one marked comment, found and edited if present, created if not, plus a job summary and a warning annotation
- [X] T067 [US5] Cache the hook environments unconditionally, with no input governing it, matching the CI capability (FR-014a)
- [X] T068 [US5] Register the event skip in `tests/test_workflow_properties.py`'s table, with the reason recording that this capability is advertised as advisory and so is exempt from principle VII by name
- [X] T069 [US5] Create `.github/workflows/advisory.yml` calling `$/.github/workflows/prek-advisory.yml`, and turn on `lint-changed-only` in this repository's own `ci.yml` call so the compensating relationship between the two is exercised rather than described
- [X] T070 [US5] Fill the `prek-advisory` section of `README.md`: the call site, the required write permission and why omitting it fails the run at startup with no job and no log, and the instruction to pass the same hook stage as to the CI capability
